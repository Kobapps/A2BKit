...
| Path | `.Linear()`, `.Arc(height, bias, jitter)`, `.BurstThenGather(radius, burstFraction, hold)`, `.Spiral(amplitude, frequency)`, or `.Path(myPath)` |
| Easing | `.Ease(A2BEaseKind.X)` — 21 kinds — or a custom `IA2BEasing` |
| Emission | `.Count(n)` / `.Count(min,max)`, `.AllAtOnce()` / `.Stagger(interval)` / `.SpreadOver(seconds)`, `.Scatter(radius)` |
...
- Package Manager ▸ A2BKit ▸ Samples ▸ **A2BKit Examples** — eight authored scenes; open one, press Play.
